...
def get_data(data, number, query):
    ...
    try:
        dates = time.strftime('%Y-%m-%d %H:%M', time.localtime(int(data[number]['date'])))
    except:
        dates = "None"
...
def connectVK(query, count):
    token = tokenvk
    r = requests.get('https://api.vk.com/method/newsfeed.search', params={'q': query, 'count': count, 'access_token': token, 'type': 1, 'v': 5.56})
